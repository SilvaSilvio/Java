...
public class GenericDAO<T> {
	...
	public static <T> List<T> findListHQL(String hql) {				
		Session session = EntityManagerHelper.getEntityManager().unwrap(Session.class);		
	...
	public static <T> List<T> findListByFieldHQL(String hql, String fieldName, String fieldValue) {				
		Session session = EntityManagerHelper.getEntityManager().unwrap(Session.class);		
		@SuppressWarnings("unchecked")
		List<T> list = session.createQuery(hql).setParameter(fieldName, fieldValue).list();
		return list;
	}

	public static <T> List<T> findListByFieldHQL(String hql, String fieldName, Long fieldValue) {				
		Session session = EntityManagerHelper.getEntityManager().unwrap(Session.class);		
		@SuppressWarnings("unchecked")
		List<T> list = session.createQuery(hql).setParameter(fieldName, fieldValue).list();
		return list;
	...
	public static <T> List<T> findListSQL(String sql) {				
		Session session = EntityManagerHelper.getEntityManager().unwrap(Session.class);		
		@SuppressWarnings("unchecked")
		List<T> list = session.createNativeQuery(sql).list();
		return list;
	}
}
